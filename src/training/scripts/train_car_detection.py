#!/usr/bin/env python3
"""
Car Detection Training Script

Script này thực hiện training model YOLOv8 cho việc phát hiện xe ô tô.
...
Usage:
    ...
    python train_car_detection.py --config ../configs/car_detection_config.yaml --resume
...
def train_car_detection(config_path, resume=False):
    """Main training function for car detection."""
    
    # Load configuration
    config = load_config(config_path)
    
    # Setup logging
    logger = setup_logging('car_detection_training')
    logger.info(f"Starting car detection training with config: {config_path}")
...
def main():
    ...
    # Start training
    try:
        results = train_car_detection(args.config, args.resume)
        print("\n=== Training Summary ===")
